...
| PHPUnit | 880+ unit tests (in-process fake TCP workers for memcached meta, Redis RESP, Ignite thin-client); memcached / Redis / Ignite integration + contract suites |
| Coverage | **≥ 90%** line coverage on `src/PureCache` plus a 40-statement buffer (`composer test:coverage`; fake TCP workers for meta/Redis/Ignite wire paths) |
| Infection | MSI ≥ 98%, covered MSI ≥ 99% (`composer test:mutation`) |
